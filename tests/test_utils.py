        ...
        "choices": [{
            "message": {
                ...
                "tool_calls": [{
                    "id": "call_1",
                    "type": "function",
                    "function": {"name": "example_tool", "arguments": '{"x": 1, "y": "test"}'}
                }]
            }
        }]
    }
...
def test_has_tool_call(tool_completion):
    ...
    assert not has_tool_call({"choices": [{"message": {"role": "assistant", "content": "Hi"}}]})
...
def test_create_literal_pydantic_model():
    ...
    assert Literal["red", "green", "blue"] == model.model_fields["value"].annotation
