        ...
        if isinstance(doc_dict, str):
            ...
            if doc_dict.get("raises"):
                ...
                for type_name, desc in doc_dict["raises"]:
                    parts.append(f"{prefix}  - `{type_name}` - {exc.description}") # Corrected exc.description
            return "\n".join(parts)
        else:
...
def _parse_docstring(obj: Any, use_getdoc: bool = True) -> Optional[dict]:
    ...
    try:
        ...
        return {k: v for k, v in result.items() if v and (not isinstance(v, list) or len(v) > 0)}
    except Exception as e:
...
def format_to_markdown(
    ...
    @cached(
        ...
        _visited: (
            ...
            show_header,
            schema,
            id(_visited) if _visited is not None else None, # Use ID for the visited set
        )
    )
    def _format_to_markdown(
        ...
        if isinstance(target, BaseModel) or (
            ...
            if code_block:
                if schema or is_class: # If schema is True or it's the class itself, show schema
                    data = {}
            ...
            for key, field_info in fields:
                ...
                if should_show_value: # Show value only if it's an instance AND schema is False
                    ...
                    value = getattr(target, key, "<missing>") # Use getattr with default for robustness

                    # Recursively format the value
                    formatted_value = _format_to_markdown(
                        value,
                        field_indent_base + (1 if not compact else 0), # Increase indent for nested values
                    ...
                    if '\n' in formatted_value:
                        ...
                        # Single line value
                        field_line_parts.append(f"{prefix}{bullet}{key}{type_info} = {formatted_value}")
        ...
        if isinstance(target, (list, tuple, set)):
            ...
            # try to dump as JSON.
            if code_block and all(isinstance(item, (dict, BaseModel)) for item in target):
                try:
                    ...
                    logger.warning(f"Failed to serialize collection to JSON code block: {e}")
            ...
            header = ""
            if show_title:
                header = f"{prefix}{bullet}**{type_name}**:" if show_types else f"{prefix}{bullet}Collection:"
            ...
            for item in target:
                ...
                items.append(f"{'  ' * item_indent}{bullet_style} {formatted_item.lstrip()}")
        ...
        if isinstance(target, dict):
            ...
            if code_block:
                ...
                if schema:
                     ...
                     for key, value in target.items():
                          ...
                          if isinstance(value, (BaseModel)) or (isinstance(value, type) and issubclass(value, BaseModel)) or is_dataclass(value) or (isinstance(value, type) and is_dataclass(value)):
                               # If value is a model/dataclass (type or instance), show its schema
                               schema_data[key] = _format_to_markdown(value, schema=True, code_block=False, show_title=False, show_bullets=False, show_docs=False, _visited=visited_copy).strip()
                          else:
            ...
            header = ""
            if show_title:
                header = f"{prefix}{bullet}**{type_name}**:" if show_types else f"{prefix}{bullet}Dictionary:"
            ...
            for key, value in target.items():
                ...
                if show_bullets:
                    items.append(f"{prefix}{'  ' * item_indent}{bullet_style} {key}: {formatted_value.lstrip()}")
                else:
                    items.append(f"{prefix}{'  ' * item_indent}{key}: {formatted_value.lstrip()}")
        ...
        if is_dataclass(target):
            is_class = isinstance(target, type) # Check if it's a dataclass type or instance
            type_name = target.__name__ if is_class else target.__class__.__name__
            ...
            if code_block: # Dataclasses don't have model_dump, so we manually build dict
                if schema or is_class: # If schema is True or it's the class itself, show schema
                    ...
                    data = {f.name: getattr(target, f.name) for f in dataclass_fields(target)}
                    # Use json.dumps for proper JSON formatting with indentation
                    json_str = json.dumps(data, indent=2)
                ...
                if should_show_value: # Show value only if it's an instance AND schema is False
                    ...
                    if '\n' in formatted_value:
                        ...
                        field_line_parts.append(f"{formatted_value}")
                         ...
                         # Single line value
                        field_line_parts.append(f"{prefix}{bullet}{f.name}{type_info} = {formatted_value}")
                else: # Schema mode or is_class: show only key and type
                    field_line_parts.append(f"{prefix}{bullet}{f.name}{type_info}")
