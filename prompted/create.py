...
class _CreateClientDeps:
    ...
    """Reference to `CLIENT_DEPS._instructor_mode`."""
    ...
    def initialize_instructor(self):
        ...
        self._instructor_mode = CLIENT_DEPS._instructor_mode
    ...
    def instructor_mode(self, mode: InstructorModeParam = "tool_call"):
        ...
        self.instructor_sync.mode = self._instructor_mode(mode)
    ...
    def instructor_async_mode(self, mode: InstructorModeParam = "tool_call"):
        ...
        self.instructor_async.mode = self._instructor_mode(mode)
...
def _parse_string_schema(schema_str: str, model_name: str, model_description: Optional[str]) -> Type[BaseModel]:
    ...
    for i, definition in enumerate(field_definitions):
        ...
        fields_config[field_name] = (actual_field_type, Field(default=None, description=f"Field '{field_name}'"))
...
def _format_compiled_messages(
    ...
    if existing_messages:
        compiled_messages.extend(format_messages(existing_messages)) # normalize_messages should handle List[Message]

    # 2. Incorporate instructions as a system message
    if instructions is not None:
        # normalize_system_prompt merges or prepends the system prompt
        compiled_messages = format_system_prompt(compiled_messages, system_prompt=instructions)
            ...
            if not actual_response_model.model_fields:
                ...
                for field_name, field_info in actual_response_model.model_fields.items():
                    ...
                    try:
                        ...
                        client.mode = CLIENT_DEPS._instructor_mode(mode)
                        field_response = client.chat.completions.create(**call_params)
                        client.mode = CLIENT_DEPS._instructor_mode("tool_call")
    ...
    def from_options(
        ...
        if all(isinstance(opt, str) for opt in options):
            ...
            SelectionModelSchema = create_model(
                ...
                selection=(literal_options_type, Field(..., description="The selected option.")),
                __base__=BaseModel,
