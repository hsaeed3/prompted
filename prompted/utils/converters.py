...
def convert_to_message(
    ...
    """
    @cached(lambda message, role="user", markdown=False, use_parts=False: 
            make_hashable((message, role, markdown, use_parts)))
    def _convert_to_message(
        ...
        if markdown:
            if not isinstance(message, str):
                try:
                    message = format_to_markdown(message)
        ...
        if use_parts:
            message = MessageContentTextPart(
                type = "text",
                text = message
            )
        return Message(
            role = role,
            content = message if not use_parts else [message]
        )
    
    return _convert_to_message(message, role, markdown, use_parts)
