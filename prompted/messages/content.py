...
class ImageContent(BaseContent):
    ...
    def process_image_input(cls, v: str) -> str:
        ...
        # Assume it's a file path and try to read and encode it
        try:
            path = Path(v)
            if not path.exists():
...
class AudioContent(BaseContent):
    ...
    def from_openai(cls, data: Dict[str, Any]) -> "AudioContent":
        ...
        return cls(
            audio=input_audio_data["data"],
            format=input_audio_data.get("format", "mp3")
        )
...
class ToolContent(BaseContent):
    ...
    @classmethod
    def from_openai_tool_call(cls, data: Dict[str, Any], result: Any = None) -> "ToolContent":
        """Creates a ToolContent instance from OpenAI tool call format."""
        import json
        
        function_data = data["function"]
        parameters = json.loads(function_data["arguments"]) if function_data["arguments"] else {}
    ...
    @classmethod
    def from_openai_tool_message(cls, data: Dict[str, Any], name: str, parameters: Dict[str, Any] = None) -> "ToolContent":
